...
public partial class FormMain : FormBase
{
        ...
        if (openFileDialog.ShowDialog() == DialogResult.OK)
        {
            ...
            try
            {
                ...
                if (fileOK)
                {
                    ...
                    //display opened file
                    DisplayFileInfo(_filePath, hardTypeString, hardType, hardVer, appType, appVer, firmVer, firmRev);
    ...
    void DisplayFileInfo(string filePath, string hardTypeString, int hardType, int hardVer, int appType, int appVer, int firmVer, int firmRev)
    {
        richTextBox1.AppendText("______________________________________________________________________" + System.Environment.NewLine);
    ...
    private void button2_Click(object sender, EventArgs e)
    {
        ...
        int hByteCount, hAddress, hRecordType, hCheckSum, hAddressMax = 0;
        ...
        else
        {
            ...
            hafFileName = hardTypeString.ToLower() + "_" +
                          HexBuffer[0x1012] + "-" + HexBuffer[0x1013] + "-" + HexBuffer[0x1014] + "-" + HexBuffer[0x1015] +
                          "-rev" + (HexBuffer[0x1016] * 256 + HexBuffer[0x1017]);
            saveFileDialog.FileName = Path.GetFileName(hafFileName);
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                using var writer = new StreamWriter(saveFileDialog.FileName);
                writer.Write(hafFile);
                DisplayFileSaved(saveFileDialog.FileName, hCheckSum, hAddressMax);
            }
        }
    }
    void DisplayFileError(int hAddressError)
    {
        richTextBox1.AppendText("______________________________________________________________________" + System.Environment.NewLine);
        richTextBox1.AppendText("FILE ERROR                                   Time: " + DateTime.Now + System.Environment.NewLine);
        richTextBox1.AppendText(System.Environment.NewLine);
        richTextBox1.AppendText("      File error at address 0x" + hAddressError.ToString("X6") + System.Environment.NewLine);
        richTextBox1.ScrollToCaret();
    }
